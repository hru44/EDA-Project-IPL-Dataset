...
--for more info refer to https://docs.oracle.com/cd/B12037_01/server.101/b10759/sql_elements002.html
...
order by salary; --always the  lower limit first, then higher limit
...
SELECT * FROM EMPLOYEES
WHERE FIRST_NAME BETWEEN 'Allan' AND 'Clara'
order by FIRST_NAME;  
...
SELECT * FROM EMPLOYEES
WHERE SALARY IN (10000, 24000,17000);--the order is not important
...
SELECT * FROM EMPLOYEES
WHERE FIRST_NAME LIKE '_a%';-- the first_name which has d in second letter
...
SELECT * FROM EMPLOYEES
ORDER BY COMMISSION_PCT ; --by default null come first in Ascending order


SELECT * FROM EMPLOYEES
ORDER BY COMMISSION_PCT desc ----by default null come last in Descending order
...
SELECT EMPLOYEE_ID , SALARY, SALARY+100 sal100
FROM EMPLOYEES
order by SALARY+100;
